...
class GapService:
    ...
    async def analyze_gaps_with_ai(
        ...
        """
        # 先跑启发式分析作为基础
        heuristic = GapService.analyze_gaps_heuristic(selected_fragments, goal)
        
        # 如果准备度已经很高，直接返回
        if heuristic["overall_readiness"] >= 85:
            return heuristic

        # 构建 prompt
        fragments_text = "\n".join(
            f"- [{f.get('type', '技能')}] {f.get('content', '')}" for f in selected_fragments
        )

        prompt = f"""用户想要完成目标：「{goal}」

职业：{profession or '未填写'}

已选碎片：
{fragments_text}

请分析：要完成「{goal}」，用户还缺什么类型的能力碎片？
...
- suggestion 要具体可操作，不要空泛
- action 必须是今天就能做的具体动作
- summary 用接地气的口语，像朋友给建议"""

        try:
            response = await _client.chat.completions.create(
                model=AI_MODEL,
                messages=[
                    {"role": "system", "content": "你是一个职业规划助手，帮用户分析能力缺口。语言接地气，像朋友聊天。"},
            ...
            return {
                "goal": goal,
                "has_enough_fragments": ai_result.get("overall_readiness", heuristic["overall_readiness"]) >= 50,
                "total_fragments": len(selected_fragments),
                "type_coverage": heuristic["type_coverage"],
                "missing_types": ai_result.get("missing_types", heuristic["missing_types"]),
                "gaps": ai_result.get("gaps", heuristic["gaps"]),
                "overall_readiness": ai_result.get("overall_readiness", heuristic["overall_readiness"]),
                "summary": ai_result.get("summary", heuristic["summary"]),
                "ai_enhanced": True,
            }

        except Exception:
            # AI失败时回退到启发式
            return {**heuristic, "ai_enhanced": False}
