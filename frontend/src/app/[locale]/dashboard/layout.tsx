...
import { PuzzleIcon, LightbulbIcon, JournalIcon, CheckIcon, SparkleIcon, CompassIcon, HandshakeIcon, HistoryIcon } from "@/components/AppIcons";
...
const navSections = [
  {
    label: '日常',
    items: [
      { href: "/dashboard/journal", label: "日志", icon: <JournalIcon size={20} />, dataTour: "nav-journal" },
      { href: "/dashboard/checkin", label: "打卡", icon: <CheckIcon size={20} />, dataTour: "nav-checkin" },
    ],
  },
  {
    label: '发现',
    items: [
      { href: "/dashboard/fragments", label: "碎片", icon: <PuzzleIcon size={20} />, dataTour: "nav-fragments" },
      { href: "/dashboard/fusion", label: "融合", icon: <SparkleIcon size={20} />, dataTour: "nav-fusion" },
  ...
  },
  {
    label: '成长',
    items: [
      { href: "/dashboard/analytics", label: "分析", icon: <span className="text-base">📊</span> },
...
export default function DashboardLayout({
  ...
  const mobileClass = (href: string) =>
    `flex flex-col items-center gap-0.5 text-xs py-1 transition-colors ${isActive(href) ? 'text-warm-accent dark:text-dark-accent font-medium' : 'text-warm-dark/50 dark:text-dark-text/50 hover:text-warm-accent dark:hover:text-dark-accent'}`;

  const desktopClass = (href: string) =>
    `w-12 h-12 flex items-center justify-center rounded-xl backdrop-blur-sm border transition-all ${isActive(href) ? 'bg-warm-accent/10 dark:bg-dark-accent/10 border-warm-accent/30 dark:border-dark-accent/30 text-warm-accent dark:text-dark-accent shadow-sm' : 'bg-white/60 dark:bg-dark-surface/60 border-warm-dark/10 dark:border-dark-border text-warm-dark/50 dark:text-dark-text/50 hover:text-warm-accent dark:hover:text-dark-accent hover:border-warm-accent/30 dark:hover:border-dark-accent/30'}`;
  ...
  return (
    <ToastProvider>
      <div className="min-h-screen flex flex-col">
        {/* 顶栏 */}
        <header className="sticky top-0 z-20 bg-warm-light/80 dark:bg-dark-bg/80 backdrop-blur-md border-b border-warm-dark/10 dark:border-dark-border">
          <div className="max-w-4xl mx-auto px-4 h-14 flex items-center justify-between">
            <Link href="/" className="font-bold text-lg text-warm-dark dark:text-dark-text">
              拼拼看Me
            </Link>
            <button
        ...
        {/* 用量提示条 */}
        <div className="max-w-4xl mx-auto w-full px-4 pt-2" data-tour="usage-bar">
          <UsageBar />
        </div>
        ...
        {/* 底部导航栏（移动端） */}
        <nav className="sticky bottom-0 z-20 bg-warm-light/90 dark:bg-dark-bg/90 backdrop-blur-md border-t border-warm-dark/10 dark:border-dark-border md:hidden">
