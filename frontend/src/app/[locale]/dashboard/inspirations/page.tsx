...
export default function InspirationsPage() {
  ...
  // 转化为碎片：调用碎片创建 API
  const handleConvertToFragment = async (insp: Inspiration) => {
    try {
      const content = [insp.title, insp.insight, insp.spark, insp.action]
        .filter(Boolean)
        .join(' | ');
      const res = await fetch('/api/fragments', {
        ...
        body: JSON.stringify({
          content,
          fragment_type: '灵感',
          tags: ['来自灵感集', `灵感ID:${insp.id}`],
          source: 'inspiration',
        }),
      });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      alert('✅ 已转化为碎片！去碎片页面查看');
    } catch (err) {
  ...
  // 用于融合：转化 + 跳转
  const handleUseForFusion = async (insp: Inspiration) => {
    try {
      const content = [insp.title, insp.insight, insp.spark, insp.action]
        .filter(Boolean)
        .join(' | ');
      ...
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      window.location.href = `/dashboard/fusion?inspiration_id=${insp.id}`;
    } catch (err) {
      alert('融合失败：' + (err instanceof Error ? err.message : '未知错误'));
    }
  };
    ...
    <div className="space-y-6">
      ...
      {!loading && !error && (
        <InspirationsList
          ...
          onConvertToFragment={handleConvertToFragment}
          onUseForFusion={handleUseForFusion}
        />
      )}
