...
// 动态导入重组件
const PuzzleFusionAnimation = dynamic(() => import('@/components/PuzzleFusionAnimation'), { ssr: false });
const ShareCard = dynamic(() => import('@/components/ShareCard'), { ssr: false });
const CapabilitySpectrum = dynamic(() => import('@/components/CapabilitySpectrum'), { ssr: false });
...
// 创建行进地图
  ...
  useEffect(() => {
    ...
    const inspirationId = params.get('inspiration_id');
    if (!inspirationId) return;

    const matched = dbFragments.find(f =>
      f.tags?.includes(`灵感ID:${inspirationId}`)
    );
    if (matched) {
      setSelectedIds(prev => {
    ...
    // 清掉 URL 参数，保持 URL 干净
    window.history.replaceState({}, '', '/dashboard/fusion');
  ...
  if (showDisclaimer) {
    return (
      <div className="fixed inset-0 z-50 bg-black/30 flex items-center justify-center p-4">
        <div className="bg-white dark:bg-dark-surface rounded-2xl p-6 max-w-sm w-full shadow-xl space-y-4">
          <h3 className="text-lg font-bold text-warm-dark dark:text-dark-text">📌 开始前，先看一眼</h3>
          <div className="space-y-2 text-sm text-warm-dark/70 dark:text-dark-text/70 leading-relaxed">
      ...
      <div id="fusion-result-card" className={`space-y-6 relative ${fusionSuccess ? 'fusion-success-glow' : ''}`}>
        ...
        <div className="p-6 rounded-2xl bg-gradient-to-br from-warm-accent/15 via-warm-accent/5 to-transparent border border-warm-accent/20">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <span className="text-sm text-warm-dark/50 dark:text-dark-text/50">{profession}</span>
              {result.profile_tag && (
                <span className="ml-2 px-2 py-0.5 bg-warm-accent/10 dark:bg-dark-accent/10 text-warm-accent text-xs rounded-full">
                  {result.profile_tag}
                </span>
              )}
              {result.confidence && (
                <span className="ml-2 text-xs text-warm-dark/40 dark:text-dark-text/40">
                  置信度 {result.confidence}%
                </span>
              )}
            </div>
          </div>
          <p className="text-lg font-bold text-warm-dark dark:text-dark-text leading-relaxed">
        ...
        <div className="space-y-3">
          <h2 className="text-sm font-medium text-warm-dark/50 dark:text-dark-text/50 tracking-wide">这些碎片，指向这几个方向。</h2>
          {result.directions.map((dir, i) => {
            const diff = DIFFICULTY_CONFIG[dir.difficulty || 'medium'];
            const isOpen = expandedDir === i;
            return (
              <div key={i} className="rounded-xl border border-warm-dark/10 dark:border-dark-border bg-white/80 dark:bg-dark-surface overflow-hidden transition-all">
                <button
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <span className="font-bold text-warm-dark dark:text-dark-text">{dir.title}</span>
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        {dir.time_to_first_result && (
                          <span className="text-xs text-warm-dark/40 dark:text-dark-text/40">⏱ {dir.time_to_first_result}</span>
                        )}
                      </div>
                    </div>
                  </div>
                  <span className={`text-warm-dark/30 dark:text-dark-text/30 transition-transform ${isOpen ? 'rotate-180' : ''}`}>▼</span>
                </button>

                {isOpen && (
                  <div className="px-4 pb-5 space-y-4 border-t border-warm-dark/5 dark:border-dark-border pt-4">
                    {(dir.why_this_works || dir.description) && (
                      <div>
                        <h4 className="text-xs font-medium text-warm-dark/40 dark:text-dark-text/40 mb-1">为什么这几块能拼在一起</h4>
                        <p className="text-sm text-warm-dark/80 dark:text-dark-text/80 leading-relaxed">
                          {dir.why_this_works || dir.description}
                        </p>
                      </div>
                    )}
                    {dir.market_hint && (
                      <div>
                        <h4 className="text-xs font-medium text-warm-dark/40 dark:text-dark-text/40 mb-1">市场参考</h4>
                        <p className="text-sm text-warm-dark/70 dark:text-dark-text/70">{dir.market_hint}</p>
                      </div>
                    )}
                          ...
                          {dir.roadmap.map((s, si) => (
                            <div key={si} className={`flex gap-3${si === 0 ? ' p-3 -mx-3 rounded-xl bg-gradient-to-r from-green-50/80 via-green-50/20 to-transparent border-l-[3px] border-green-400 shadow-sm' : ''}`}>
                              <div className="flex flex-col items-center">
                                <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold text-white relative ${si === 0 ? 'bg-green-500 shadow-md shadow-green-200' : si === 1 ? 'bg-warm-accent/60 dark:bg-dark-accent/60' : 'bg-warm-accent/30 dark:bg-dark-accent/30'}`}>
                                  {s.step}
                                  {si === 0 && (
                                ...
                                {si < (dir.roadmap?.length || 0) - 1 && (
                                  <div className="w-0.5 flex-1 bg-warm-dark/10 dark:bg-dark-border mt-1" />
                                )}
                              </div>
                              <div className="pt-0.5">
                                <span className={`text-xs ${si === 0 ? 'text-green-600 font-medium' : 'text-warm-dark/40 dark:text-dark-text/40'}`}>{si === 0 ? `✅ ${s.time}` : s.time}</span>
                                <p className={`text-sm ${si === 0 ? 'text-warm-dark dark:text-dark-text font-medium' : 'text-warm-dark/80 dark:text-dark-text/80'}`}>{s.action}</p>
                              </div>
                            </div>
                          ))}
                        </div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {dir.used_fragments.map((f, j) => (
                        <span key={j} className="px-2 py-0.5 bg-warm-accent/10 dark:bg-dark-accent/10 text-warm-accent/80 text-xs rounded-full">
                          🧩 {f}
                        </span>
                      ))}
                    </div>
                    <div className="p-4 rounded-xl bg-gradient-to-r from-warm-accent/15 to-warm-accent/5 border-2 border-warm-accent/20 shadow-sm">
                      ...
                      <p className="text-sm font-medium text-warm-dark dark:text-dark-text mt-1">{dir.next_action}</p>
                    </div>
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        ...
                        className="py-2.5 px-4 bg-warm-border hover:bg-warm-border/80 text-warm-dark dark:text-dark-text rounded-xl text-sm font-medium transition-colors"
                        title="极简文字版"
                      >
                        📝 文字版
                      </button>
        ...
        {result.user_domains && result.user_domains.length > 0 && (
          <div className="p-4 rounded-2xl bg-white/60 dark:bg-dark-surface border border-warm-dark/10 dark:border-dark-border">
            <h3 className="text-xs font-medium text-warm-dark/40 dark:text-dark-text/40 mb-2">📍 感知到你的领域</h3>
            <div className="flex flex-wrap gap-1.5">
              {result.user_domains.map((domain, i) => (
                <span key={i} className="px-2.5 py-1 bg-warm-accent/8 dark:bg-dark-accent/10 text-warm-accent/80 text-xs rounded-full border border-warm-accent/15">
                  {domain}
                </span>
              ))}
            </div>
        ...
        {/* 整体洞察 */}
        <div className="p-5 rounded-2xl bg-warm-light/60 dark:bg-dark-surface border border-warm-dark/5 dark:border-dark-border">
          <h3 className="text-sm font-medium text-warm-dark/50 dark:text-dark-text/50 mb-2">💡 整体洞察</h3>
          <p className="text-sm text-warm-dark/80 dark:text-dark-text/80 leading-relaxed">{result.insight}</p>
        </div>

        {/* 能力缺口 */}
        {result.skill_gaps && result.skill_gaps.length > 0 && (
          <div className="p-5 rounded-2xl bg-white/60 dark:bg-dark-surface border border-warm-dark/10 dark:border-dark-border">
            <h3 className="text-sm font-medium text-warm-dark/50 dark:text-dark-text/50 mb-3">🔧 还差这几块碎片</h3>
            <div className="space-y-2">
              {result.skill_gaps.map((gap, i) => (
                <div key={i} className="flex items-center gap-2 text-sm text-warm-dark/70 dark:text-dark-text/70">
                  <span className="text-warm-dark/30 dark:text-dark-text/30">🧩</span>
                  <span>{gap}</span>
                </div>
              ))}
            </div>
              ...
              {result.fragment_connections.map((conn, i) => (
                <div key={i} className="p-3 rounded-xl bg-white/70 border border-purple-100/50">
                  <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                    <span className="px-2 py-0.5 bg-warm-accent/10 dark:bg-dark-accent/10 text-warm-accent text-xs rounded-full">{conn.fragment_a}</span>
                    <span className="text-purple-400 text-xs">×</span>
                    <span className="px-2 py-0.5 bg-warm-accent/10 dark:bg-dark-accent/10 text-warm-accent text-xs rounded-full">{conn.fragment_b}</span>
                  </div>
                  <p className="text-sm text-warm-dark/70 dark:text-dark-text/70 leading-relaxed">💡 {conn.connection}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* 分享卡片 */}
        <div className="border-t border-warm-dark/10 dark:border-dark-border pt-6">
          <h3 className="text-sm font-medium text-warm-dark/50 dark:text-dark-text/50 mb-4 text-center">📸 做个分享卡片</h3>
          <ShareCard
        ...
        <div className="flex items-center justify-center gap-3 pt-2 pb-4">
          ...
          <button
            ...
            className={`px-5 py-2 rounded-xl text-sm font-medium transition-all duration-300 ${
              copied
                ...
                : 'bg-warm-dark/10 text-warm-dark dark:text-dark-text hover:bg-warm-dark/20'
            }`}
          >
            {copied ? '✅ 已复制' : '📋 复制一份'}
          </button>
          <button
            onClick={handleReset}
            className="px-5 py-2 bg-warm-dark/5 text-warm-dark/60 dark:text-dark-text/60 rounded-xl text-sm font-medium hover:bg-warm-dark/10 transition-colors"
          >
            🔄 再看看别的
          </button>
        </div>
