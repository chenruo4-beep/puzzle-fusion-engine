...
export default function DashboardHome() {
  ...
  return (
    <div className="space-y-6">
      {/* ====== 欢迎区 ====== */}
      <div className="rounded-2xl bg-gradient-to-br from-warm-accent/15 via-warm-light to-warm-light border border-warm-accent/20 p-5">
        <h1 className="text-2xl font-bold text-warm-dark">
          {greeting} 👋
        </h1>
        <p className="text-sm text-warm-dark/50 mt-1">{todayStr}</p>
        <p className="text-sm text-warm-dark/70 mt-2 italic">
          {QUOTES[quoteIndex]}
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <Link
          ...
          className="rounded-xl bg-white/80 border border-warm-dark/10 p-4 text-center hover:border-warm-accent/40 hover:shadow-sm transition-all group"
        >
          <div className="text-2xl mb-1 group-hover:scale-110 transition-transform">🧩</div>
          <div className="text-sm font-medium text-warm-dark">添加碎片</div>
        </Link>
        <Link
        ...
        >
          <div className="text-2xl mb-1 group-hover:scale-110 transition-transform">🔮</div>
          <div className="text-sm font-medium text-warm-dark">开始融合</div>
        </Link>
        <Link
        ...
        >
          <div className="text-2xl mb-1 group-hover:scale-110 transition-transform">✅</div>
          <div className="text-sm font-medium text-warm-dark">今日打卡</div>
        </Link>
        <Link
        ...
        >
          <div className="text-2xl mb-1 group-hover:scale-110 transition-transform">📝</div>
          <div className="text-sm font-medium text-warm-dark">写日记</div>
        </Link>
      </div>
      ...
      {/* ====== 拼图片连接率圆环 ====== */}
      {!loading && (
        <div className="rounded-2xl bg-white/80 border border-warm-dark/10 p-5 flex flex-col items-center">
          <h3 className="text-sm font-semibold text-warm-dark/60 mb-4">🔗 拼图片连接率</h3>
          <CompletionRing
            ...
            strokeWidth={11}
          />
          <p className="text-xs text-warm-dark/30 mt-3 text-center max-w-[240px]">
      ...
      {!loading && gapData && gapData.gaps.length > 0 && (
        <div className="gap-section rounded-2xl bg-white/80 border border-warm-dark/10 p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-warm-dark">🔍 缺口识别</h3>
            <span className="text-xs text-warm-dark/30">{gapData.gap_count} 个潜在缺口</span>
          </div>
          ...
          <p className="text-xs text-warm-dark/30 mt-4 text-center">
            {gapData.message}
          </p>
          {gapData.gap_count > 6 && (
            <Link
      ...
      {!loading && clusters.length > 0 && (
        <div className="rounded-2xl bg-white/80 border border-warm-dark/10 p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-warm-dark">🧠 智能组块推荐</h3>
            <span className="text-xs text-warm-dark/30">AI 自动聚类</span>
          </div>
      ...
      <div className="rounded-2xl bg-white/80 border border-warm-dark/10 p-5">
        <div className="flex items-center justify-between mb-3">
          <h3 className="font-semibold text-warm-dark">最近活动</h3>
        </div>
        {loading ? (
          <div className="space-y-3">
            {[1, 2, 3].map(i => (
              <div key={i} className="animate-pulse h-10 bg-warm-dark/5 rounded-lg" />
            ))}
          </div>
        ) : activities.length === 0 ? (
          <div className="text-center py-8 text-warm-dark/40">
          ...
          <div className="space-y-2">
            {activities.map(a => (
              <div key={a.id} className="flex items-center gap-3 text-sm py-2 border-b border-warm-dark/5 last:border-0">
                ...
                <span className="text-warm-dark/70 truncate flex-1">{a.title}</span>
      ...
      <div className="flex items-center gap-3 px-4 py-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <span className="text-sm font-semibold text-warm-dark truncate">{name}</span>
            <span
          ...
          <p className="text-xs text-warm-dark/45 mt-0.5">{description}</p>
        </div>
        {/* 展开箭头 */}
        <span
          className="shrink-0 text-warm-dark/30 text-sm transition-transform duration-200"
        ...
        <div className="px-4 pb-3 space-y-1.5">
          {fragments.map((f) => (
            ...
            >
              ...
              <span className="text-warm-dark/70 truncate flex-1">{f.content}</span>
              {f.quality_score > 0 && (
                <span className="shrink-0 text-[10px] text-warm-dark/25">
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-warm-dark/50 mt-0.5">{badge.desc}</p>
      </div>
    </div>
  );
});
